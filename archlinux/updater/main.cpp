...
void* execute_update_thread(void* /*arg*/) {
    ...
    // 2. CURRENT VERSION
    try {
        std::string version_output = run_command("cat /home/$USER/claudemods-distribution-iso-creator-Beta/archlinux/versionarch.txt");
    ...
    if (strcmp(detected_distro, "arch") == 0 || strcmp(detected_distro, "cachyos") == 0) {
        try {
            std::string version_output = run_command(
                "cat /home/$USER/claudemods-distribution-iso-creator-Beta/archlinux/version/versionarch.txt");
    ...
    silent_command("rm -rf /home/$USER/claudemods-distribution-iso-creator-Beta/archlinux/claudemods-distro-iso-creator-arch");
    
    // ARCH AND CACHYOS INSTALLATION
    if (strcmp(detected_distro, "arch") == 0 || strcmp(detected_distro, "cachyos") == 0) {
        silent_command("cp /home/$USER/claudemods-distribution-iso-creator-Beta/archlinux/version/versionarch.txt /home/$USER/claudemods-distribution-iso-creator-Beta");
        silent_command("cd /home/$USER/claudemods-distribution-iso-creator-Beta/archlinux/ && g++ -std=c++23 -Wl,--format=binary -Wl,build-image-arch-img.zip -Wl,calamares-files.zip -Wl,claudemods.zip -Wl,--format=default main.cpp -o claudemods-distro-iso-creator-arch >/dev/null 2>&1");
        silent_command("cp /home/$USER/claudemods-distribution-iso-creator-Beta/archlinux/claudemods-distro-iso-creator-arch /home/$USER/claudemods-distribution-iso-creator-Beta >/dev/null 2>&1");
    }
        
    // GET INSTALLED VERSION
    try {
        std::string installed_version_output = run_command("cat /home/$USER/claudemods-distribution-iso-creator-Beta/archlinux/versionarch.txt");
...
int main() {
    ...
    if (response == 'y' || response == 'Y') {
        system("cd /home/$USER/claudemods-distribution-iso-creator-Beta && sudo ./claudemods-distro-iso-creator-arch");
    }
